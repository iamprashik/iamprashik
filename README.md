# 👋 Hi, I'm Prashik Koirala

### 💻 Software Developer | Computer Science Graduate

I'm a Computer Science graduate focused on building practical software and learning how systems work under the hood.

I enjoy developing applications with **Java, Python, JavaScript, and SQL**, with interests in backend development, distributed systems, cybersecurity, and software engineering.

---

## 🛠️ Tech Stack

**Languages**

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge\&logo=openjdk\&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge\&logo=python\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge\&logo=javascript\&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge\&logo=postgresql\&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge\&logo=html5\&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge\&logo=css3\&logoColor=white)

**Tools & Technologies**

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge\&logo=git\&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge\&logo=github\&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge\&logo=mysql\&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge\&logo=visual-studio-code\&logoColor=white)

---

## 🚀 Featured Projects

### 🔐 End-to-End Encrypted Chat

A secure client-server messaging application exploring encrypted communication and secure data exchange.

**Tech:** Python • Cryptography • Networking

### 🌐 Distributed File Retrieval System

A distributed system that retrieves and reconstructs files from multiple servers using SHA-256 hashing.

**Tech:** Python • Distributed Systems • Networking

### 🖥️ Process Monitor

A desktop system-monitoring application that displays system resources including CPU, memory, processes, threads, handles, drives, and GPU memory.

**Tech:** Python • Operating Systems • GUI

### 🐺 Wolf-Sheep Simulation

An agent-based ecosystem simulation exploring interactions between predators, prey, and other agents.

**Tech:** Python • Mesa • Agent-Based Modeling

### 🧮 iOS-Inspired Calculator

A responsive calculator inspired by the iOS calculator interface.

**Tech:** HTML • CSS • JavaScript

---

## 🌱 Currently Building & Learning

* ☕ Java & Data Structures
* 🧠 Algorithms & Problem Solving
* 🌐 Backend Development
* 🔐 Cybersecurity & Cryptography
* 🗄️ Databases & SQL
* ☁️ Cloud & Distributed Systems

---

## 📊 GitHub Stats

![Prashik's GitHub Stats](https://github-readme-stats.vercel.app/api?username=iamprashik\&show_icons=true\&hide_border=true\&count_private=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=iamprashik\&layout=compact\&hide_border=true)

---

## 🤝 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Prashik%20Koirala-0A66C2?style=for-the-badge\&logo=linkedin\&logoColor=white)](https://www.linkedin.com/in/prashik-koirala-b6a64b3b0/)

[![Email](https://img.shields.io/badge/Email-iamprashikkoirala%40gmail.com-EA4335?style=for-the-badge\&logo=gmail\&logoColor=white)](mailto:iamprashikkoirala@gmail.com)


---

### 💡 Build. Learn. Improve. Repeat.

<!---
iamprashik/iamprashik is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
